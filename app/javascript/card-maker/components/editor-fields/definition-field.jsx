...
class DefinitionField extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
    }
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      items[i] = (
        <li key={i}>
          <input
            type='text'
            className={[styles.textInput, styles.textInputTextField].join(' ')}
            onChange={(e) => this.props.setDefinitionText(i, e.target.value)}
            value={curVal}
          />
        </li>
      )
...
export default fieldWrapper(DefinitionField)
